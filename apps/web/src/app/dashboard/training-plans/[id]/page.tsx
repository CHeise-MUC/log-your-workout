...
export default function TrainingPlanDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`http://localhost:3001/training-plans/${planId}`, { headers }).then((r) => {
        if (!r.ok) throw new Error("Plan nicht gefunden");
        return r.json();
      }),
      fetch("http://localhost:3001/exercises", { headers }).then((r) => r.json()),
    ])
  ...
  async function handleAddExercise(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`http://localhost:3001/training-plans/${planId}/exercises`, {
        method: "POST",
        headers: {
